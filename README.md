...
33 tests, covering column detection, metrics, data quality checks, all
...
- **A serial number column got mistaken for revenue** — found via a
  real user upload of a course-catalog file (`S.No`, `Platform`,
  `Course`, `Direct Course Link`) with no actual revenue data at all.
  `S.No` (1, 2, 3...16) was the only numeric column, so the "largest
  sum" fallback picked it and reported a fabricated "Total Revenue"
  that was literally `1+2+...+16`. Fixed two ways: an exact-match
  denylist of known ID column names (`s.no`, `id`, `index`, etc.), and
  a statistical check that catches any perfect 1-step sequence
  regardless of the column's name. The tool now correctly refuses to
  generate a report and explains that no revenue column was found,
  rather than fabricating one.
- **Charts became unreadable with many categories/regions** — a
  40-category dataset produced a pie chart with 40 tiny, overlapping
  slivers (and the color palette only has a handful of distinct
  colors, so slices started repeating and became indistinguishable).
  Fixed by capping charts (both matplotlib and Excel's native charts)
  to the top 7 categories / top 11 regions plus a summed "Other"
  bucket — while every underlying number (totals, insights, full data
  tables) still uses the complete, uncapped data.

All seven are covered by permanent regression tests, and a 12-schema
